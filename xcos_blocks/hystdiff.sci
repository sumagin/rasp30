function [x,y,typ]=hystdiff(job,arg1,arg2)
    // Copyright INRIA
    x=[];y=[];typ=[];
    select job
    case 'plot' then
    ...
    case 'set' then
        ...
        while %t do
            [ok,Vbias, exprs]=scicos_getvalue('Set hysteretic differentiator buffer block parameters',['OTA Vbias (V)'],list('vec',-1),exprs)
...
//        model.state=zeros(1,1)
        ...
        exprs=[string(Vbias)] // exprs MUST BE semicolon separated & sci2exp() also converts an expression to string
