...
case 'set' then
  ...
  while %t do
    [ok,prt,exprs]=scicos_getvalue([msprintf(gettext("Set %s block parameters"),"ADC");" ";gettext("Regular output port")],..
	    gettext("Port number"),list('vec',1),exprs)
    if ~ok then break,end
    prt=int(prt)
    if prt<=0 then
      block_parameter_error(msprintf(gettext("Wrong value for ''ADC Number'' parameter: %d."),prt), ..
