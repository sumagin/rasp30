function [x,y,typ]=integrator_nmirror(job,arg1,arg2)
    ...
    case 'set' then
        ...
        model=arg1.model;
        while %t do
            [ok,in_out_num,ibias,nibias,cap1,nmirror_bias,exprs]=getvalue('Set FG OTA Parameters',['Number of FG OTAs';'Ota0 bias (A)';'Ota1 bias (A)';'Input Cap 64fF [1-6X]';'Compensate Ibias (A)'],list('vec',1,'vec',-1,'vec',-1,'vec',-1,'vec',-1),exprs)
            ...
            if ok then
                ...
                model.rpar=[ibias,nibias,cap1,nmirror_bias]
    ...
    case 'define' then
        ...
        exprs=[sci2exp(in_out_num);sci2exp(ibias);sci2exp(nibias);sci2exp(cap1);sci2exp(nmirror_bias)]
