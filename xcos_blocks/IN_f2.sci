...
function [x,y,typ]=IN_f2(job,arg1,arg2)
    x=[];y=[];typ=[]
    select job
    case 'plot' then
    ...
    case 'set' then
        ...
        while %t do
            [ok,prt,buftype,exprs]=scicos_getvalue([msprintf(gettext("Set %s block parameters"), "IN_f2");" "; ..
            gettext("Regular input port"), "Port Number", ..
            gettext("Buffer Type")], "a: Analog BUffer d: Digital buffer t: Tgate",..
    ...
    case 'define' then
        ...
        exprs=[sci2exp(prt) ;sci2exp(buftype)]
        gr_i=' '
