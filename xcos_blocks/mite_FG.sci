function [x,y,typ]=mite_FG(job,arg1,arg2)
    // Copyright INRIA
    x=[];y=[];typ=[];
    select job
    case 'plot' then
    ...
    case 'set' then
        ...
        while %t do
            [ok, in_out_num,mite_current, exprs]=scicos_getvalue('New Block Parameter',['number of blocks','MITE_current'],list('vec',-1,'vec',-1),exprs)
            ...
            if ok then
                ...
                model.opar=list(in_out_num,mite_current) //always a list
    ...
    case 'define' then
        ...
        model.sim=list('mite_FG_c',5)
        model.in=[-1;-1;-1] //first/row dimension
        model.in2=[-1;-1;-1] //second/column dimension
        model.intyp=[-1;-1;-1] 
        model.out=[-1]
        model.out2=[-1]
        model.outtyp=[-1]
        ...
        exprs=[sci2exp(in_out_num);sci2exp(mite_current)] // exprs MUST BE semicolon separated & sci2exp() also converts an expression to string
        gr_i=['txt='' Blank '';';'xstringb(orig(1),orig(2),txt,sz(1),sz(2),''fill'')']
        x=standard_define([10 5],model, exprs,gr_i) //Numbers define the width and height of block
    end
endfunction
