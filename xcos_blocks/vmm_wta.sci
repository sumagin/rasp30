function [x,y,typ]=vmm_wta(job,arg1,arg2)
    // Copyright INRIA
    x=[];y=[];typ=[];
    select job
    case 'plot' then
    ...
    case 'set' then
        ...
        while %t do
            ...
            if ipsize(1) ~= size(pfetbias',1) then
                message('The row dimension of the input vector and the number of pfet biases are not equal.');
                ok=%f;
            end
