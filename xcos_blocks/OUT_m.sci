...
function [x,y,typ]=OUT_m(job,arg1,arg2)
x=[];y=[];typ=[]
select job
case 'plot' then
...
//    xfpolys(x,y,coli);
//    xset('pattern',coli)
//    xstringb(xo,yo,string(prts),sz(1)/1.5,sz(2))
//    xset('pattern',pcoli)
//    xstringb(xo,yo,string(prts),sz(1)/1.5,sz(2))
//  else
//    xstringb(xo,yo,string(prts),sz(1)/1.5,sz(2))
...
case 'set' then
  ...
  while %t do
    [ok,prts,prt,exprs]=scicos_getvalue([msprintf(gettext("Set %s block parameters"), "OUT_m");" "; ..
      gettext("Regular output port")], ["Port Number","Number of Ports"], ..
...
case 'define' then
  ...
  exprs=[sci2exp(prts);sci2exp(prt)]
  gr_i=['txt='' Output '';';'xstringb(orig(1),orig(2),txt,sz(1),sz(2),''fill'')']
