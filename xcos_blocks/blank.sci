function [x,y,typ]=blank(job,arg1,arg2)
    // Copyright INRIA
    x=[];y=[];typ=[];
    select job
    case 'plot' then
    ...
    case 'set' then
        ...
        while %t do
            [ok,your_variable1, your_variable2, exprs]=scicos_getvalue('Blank Block Title in Dialog Box',['Your Parameter Discription 1 (units)';'Your Parameter Discription 2 (units)'],list('str',-1,'vec',1),exprs)
