function [x,y,typ]=sinh_block(job,arg1,arg2)
    // Copyright INRIA
    x=[];y=[];typ=[];
    select job
    case 'plot' then
    ...
    case 'set' then
        ...
        model=arg1.model;
        while %t do
            [ok,in,out,xx,rpar,exprs]=getvalue('Set SINH NEW block parameters',..
    ...
    case 'define' then
        ...
        gr_i=['txt=''SINH '';';'xstringb(orig(1),orig(2),txt,sz(1),sz(2),''fill'')']
