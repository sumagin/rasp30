...
function [x,y,typ]=IN_m(job,arg1,arg2)
x=[];y=[];typ=[]
select job
case 'plot' then
...
case 'set' then
  ...
  while %t do
    [ok,prts,prt,exprs]=scicos_getvalue([msprintf(gettext("Set %s block parameters"), "IN_f");" "; ..
      gettext("Regular input port")], ["Port Number","Number of Ports"], ..
...
case 'define' then
  ...
  gr_i=['txt='' Input '';';'xstringb(orig(1),orig(2),txt,sz(1),sz(2),''fill'')']
