...
function [x,y,typ]=OUT_f2(job,arg1,arg2)
    x=[];y=[];typ=[];
    select job
    case 'plot' then
        xf=60
    ...
    case 'set' then
        ...
        while %t do
        [ok,prt,buftype,exprs]=scicos_getvalue([msprintf(gettext("Set %s block parameters"),"OUT_f2");" ";gettext("Regular output port"),..
        gettext("Port number"),..
        gettext("Buffer Type")], "a: Analog BUffer d: Digital buffer",..
    ...
    case 'define' then
        ...
        exprs=[string(prt) string(buftype)]
        gr_i=' '
