function [x,y,typ]=nmirror_vmm(job,arg1,arg2)
    // Copyright INRIA
    x=[];y=[];typ=[];
    select job
    case 'plot' then
    ...
    case 'set' then
        ...
        while %t do
            [ok, in_out_num,nmirror_bias, exprs]=scicos_getvalue('New Block Parameter',['number of blocks','nmirror_bias'],list('vec',-1,'vec',-1),exprs)
            ...
            if ok then
                ...
                model.opar=list(in_out_num,nmirror_bias) //always a list
    ...
    case 'define' then
        ...
        model.sim=list('nmirror_vmm_c',5)
        ...
        exprs=[sci2exp(in_out_num);sci2exp(nmirror_bias)] // exprs MUST BE semicolon separated & sci2exp() also converts an expression to string
